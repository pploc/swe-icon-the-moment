...
async function createRenderer() {
  ...
  return applyMarkdownPlugins(
    new MarkdownIt({
      ...markdownOptions,
      highlight(code, lang) {
        // Diagram fences are handed to the browser; everything else is
        // highlighted here so no highlighter ships to the client.
...
/**
 * Second pass: now that every title is known, cross-link follow-ups and
 * [[slug]] references, then render. Splitting this from parsing is what lets a
 * question link to one that appears later in the walk.
 */
function renderQuestions(md, questions) {
  const index = buildIndex(questions)
  const base = process.env.VITE_BASE ?? '/'

  for (const question of questions) {
    const prepared = prepareBody(question.raw, {
      index,
      base,
      selfSlug: question.slug,
      onError: (message) => fail(question.file, message),
    })

    const sections = splitSections(prepared)
    if (!sections.answer) warn(question.file, 'has no answer section yet')
    ...
    question.searchBody = searchBody
    question.excerpt = searchBody.slice(0, 180).trim()
    question.html = {
...
async function emit(groups, topics, questions) {
  ...
  const meta = questions.map(
    ({ html: _html, raw: _raw, searchBody: _searchBody, file: _file, ...rest }) => rest,
  )
